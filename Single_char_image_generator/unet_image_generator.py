...
def apply_perspective_transform(img,position,chars,best_color,font,output_image_size=64,mask_size=80,max_degree=10):
	...
	draw1.text((5, 5), chars, (255, 255, 255), font=font)
...
def generate_single_word(img_path,color_lib,font_path,font_unsupport_chars,chars,cf,output_image_size=64,mask_size=80,temp_word_size=70):
	...
	img, mask = apply_perspective_transform(img,(x1,y1),chars,best_color,font)
	...
	with open(labels_path,'a',encoding='utf-8') as f:
		...
		for k in tqdm(range(0,len(ch_list))):
			...
			for i in range(cf.num_per_word):
				...
				gen_img_path = os.path.join('/home/jovyan/esun_handwritten/Traditional-Chinese-Handwriting-Dataset/unet_data/images/train/train/',save_img_name)
				gen_mask_path = os.path.join('/home/jovyan/esun_handwritten/Traditional-Chinese-Handwriting-Dataset/unet_data/labels/train/train/',save_mask_name)
