...
def apply_perspective_transform(img,position,chars,best_color,font,word_position,output_image_size=64,mask_size=80,max_degree=10):
	'''
	input:
	...
	draw.text(word_position, chars, best_color, font=font)

	# apply perspective transform on mask
	mask = np.array(mask)
    
	# draw text on mask
	mask1 = Image.new('RGBA', (mask_size,mask_size), (0,0,0,0)) 
	draw1 = ImageDraw.Draw(mask1)
	draw1.text(word_position, chars, (255, 255, 255), font=font)
	...
	mask1 = cv2.warpPerspective(mask1,M,(output_image_size,output_image_size))

	mask1 = Image.fromarray(mask1)

	#paste mask on bg and do final_crop 
	mask_img = Image.fromarray(np.zeros([mask_size,mask_size,3], dtype='uint8'))
	img.paste(mask,(x1,y1),mask)
	img = img.crop((x1,y1,x1+output_image_size,y1+output_image_size))
    
	mask_img.paste(mask1,(x1,y1),mask1)
	mask_img = mask_img.crop((x1,y1,x1+output_image_size,y1+output_image_size))

	return img, mask_img

def generate_single_word(img_path,color_lib,font_path,font_unsupport_chars,chars,cf,output_image_size=64,mask_size=80,temp_word_size=50):
	...
	temp_word_size=random.randint(35, 55)
	if temp_word_size<=40:
		word_position = (25,25)
	elif temp_word_size>40 and temp_word_size<=45:
		word_position = (20,20)
	elif temp_word_size>45 and temp_word_size<=50:
		word_position = (15,15)
	else:
		word_position = (10,10)
	...
	img, mask = apply_perspective_transform(img,(x1,y1),chars,best_color,font,word_position)

	return img,mask,chars
...
def get_word_list(word_txt_path):
	...
	with open(word_txt_path,'r',encoding="utf8") as f:
		...
		voc_list = [char.split(',')[0] for char in all_list]
		num_list = [int(char.split(',')[1]) for char in all_list]
	return voc_list, num_list
...
if __name__ == '__main__':
	...
	parser.add_argument('--fonts_path',type=str, default='./fonts/tra_chinese',
						help='The font used to generate the picture')

	parser.add_argument('--chars_file',  type=str, default='class_count.txt',
	...
	ch_list, num_list = get_word_list(cf.chars_file)
	...
	with open(labels_path,'a',encoding='utf-8') as f:
		...
		for k in tqdm(range(0,len(ch_list))):
			...
			for i in range(num):
				...
				gen_img, gen_mask, char = generate_single_word(img_path,color_lib,font_path,font_unsupport_chars,char,cf)  
				...
				if i%5==1:
					...
					image_arr_mask = np.array(gen_mask) 
					gen_mask = apply_blur_on_output(image_arr_mask)            
					gen_mask = Image.fromarray(np.uint8(gen_mask))
				#模糊图像，模拟小图片放大的效果
				elif i%5==2:
					image_arr = np.array(gen_img) 
					...
					gen_img = Image.fromarray(np.uint8(gen_img))
					image_arr_mask = np.array(gen_mask) 
					gen_mask = apply_prydown(image_arr_mask)
					gen_mask = Image.fromarray(np.uint8(gen_mask))
				#左右运动模糊
				elif i%5==3:
					image_arr = np.array(gen_img)
					...
					gen_img = Image.fromarray(np.uint8(gen_img))       
					image_arr_mask = np.array(gen_mask)
					gen_mask = apply_lr_motion(image_arr_mask)
					gen_mask = Image.fromarray(np.uint8(gen_mask)) 
				#上下运动模糊       
				elif i%5==4:
					image_arr = np.array(gen_img)
					...
					gen_img = Image.fromarray(np.uint8(gen_img)) 
					image_arr_mask = np.array(gen_mask)
					gen_mask = apply_up_motion(image_arr_mask)        
					gen_mask = Image.fromarray(np.uint8(gen_mask))

# 				save_img_name = f"{str(i).zfill(6)}_{char}.jpg"
				save_mask_name = f"{str(i).zfill(6)}_{char}.jpg"
# 				gen_img_path = os.path.join(cf.output_dir, 'train', save_img_name)
				gen_mask_path = os.path.join(cf.output_dir, 'labels',save_mask_name)
# 				gen_img.save(gen_img_path)
				gen_mask.save(gen_mask_path)
				start +=1
